...
- **Güncel `pathnames` haritası** (`src/i18n/routing.ts`):
  ...
  - **Aynı `force-static` + iç içe dinamik segment sorunu `next-intl`'in `Link`'ini de etkiliyor**: `urunler/[slug]/page.tsx` gibi rotalarda `<Link href="/urunler">` (locale prop'suz, implicit inference'a bırakılmış) her zaman `defaultLocale`'e (`/tr/urunler`) düşüyor — EN sayfasındayken bile. Bu yüzden bu rotalarda (ve `not-found.tsx` gibi ona bağlı boundary dosyalarında) `Link`'e her zaman açık `locale={locale}` geçilmeli. `not-found.tsx` server component olarak `params` ALMADIĞI için (Next.js bu convention dosyasına params geçmiyor) client component yapılıp `useLocale()`/`useParams()` ile locale okunuyor — bu da yalnızca yukarıdaki `NextIntlClientProvider locale={locale}` fix'i sayesinde güvenilir
